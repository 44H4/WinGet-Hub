...
echo --------------------------------------------------------
echo             Winget Hub Installer - Foxit PDF
echo --------------------------------------------------------
echo Made by : 44H4
...
REM Installing Foxit PDF
echo Installing Foxit PDF...
winget install -e --id foxitsoftware.Foxitreader
echo Opening the WinGet Hub website (minimized) for more information...
start /MIN powershell -command "Start-Process 'https://winget-hub.pages.dev'"
REM Check installation status
if %ERRORLEVEL% EQU 0 (
    echo Foxit PDF installed successfully.
) else (
    echo Failed to install Foxit PDF. Please check for errors above.
)
pause