...
echo --------------------------------------------------------
echo        Winget Hub Installer - Everything Search
echo --------------------------------------------------------
echo Made by : 44H4
...
REM Installing Everything Search
echo Installing Everything Search...
winget install -e --id voidtools.Everything
echo Opening the WinGet Hub website (minimized) for more information...
start /MIN powershell -command "Start-Process 'https://winget-hub.pages.dev'"
REM Check installation status
if %ERRORLEVEL% EQU 0 (
    echo Everything Search installed successfully.
) else (
    echo Failed to install Everything Search. Please check for errors above.
)
pause